package ppfootballmanager.services;
...
import ppfootballmanager.models.EventImpl;
import ppfootballmanager.models.EventType;
import ppfootballmanager.models.GoalEventImpl;
...
private int calculateTeamStrength(ITeam team) {
    ...
    private void generateOtherEvents(MatchImpl match, ITeam homeTeam, ITeam awayTeam) {
        ...
        for (int i = 0; i < yellowCards; i++) {
            ...
            if (player != null) {
                IEvent cardEvent = new EventImpl(EventType.YELLOW_CARD, minute, player, team);
        ...
        if (random.nextInt(5) == 0) { // 20% chance
            ...
            if (player != null) {
                IEvent cardEvent = new EventImpl(EventType.RED_CARD, minute, player, team);
                match.addEvent(cardEvent);
            }
        }
    }
